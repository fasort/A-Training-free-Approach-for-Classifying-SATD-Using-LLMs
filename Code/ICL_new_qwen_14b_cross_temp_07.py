...
# 用于检索的数据集
df_retrieval = pd.read_csv('/root/xinglin-data/satd/retrieval_data/OBrien_789_v2_retrieval.csv')
...
df_retrieval['satd_type'] = df_retrieval['satd_type'].map(retrieval_label_mapping)

df = df[['context','prompt_context','Category','Project']]
df_retrieval = df_retrieval[['context','prompt_context','satd_type','fold']]
df_retrieval = df_retrieval.rename(columns={'satd_type': 'Category', 'fold': 'Project'})
...
for NUM_EXAMPLES_IN_PROMPT in num_instances:
    ...
    file_name = 'Results/' + DATASET + '_Input-new-cross_' + INPUT + '_' + checkpoint.split('/')[-1] + icl_name
