...
checkpoint = "/root/xinglin-data/satd/models/LLM-Research/gemma-2-9b-it"

model = LLM(model="/root/xinglin-data/satd/models/LLM-Research/gemma-2-9b-it",gpu_memory_utilization=0.9)

tokenizer = GemmaTokenizerFast.from_pretrained("/root/xinglin-data/satd/models/LLM-Research/gemma-2-9b-it")
...
def generate_prompt_by_top_n_items(init_prompt, test_context, top_n_items, example_num, data):
    prompt = init_prompt
    # example_box = {'IMPLEMENTATION':[],'TEST':[],'DEFECT':[],'DESIGN':[],'DOCUMENTATION':[]}
    ...
    for label in label_list:
        ...
        for indx,similarity in top_n_items:
            ...
            if data['label'][indx] == label and cnt <= example_num:
    ...
    prompt += test_context + '\n'
    prompt += 'There is only one type. You only need answer the type.<end_of_turn>\n<start_of_turn>model\n'
...
for NUM_EXAMPLES_IN_PROMPT in num_instances:
    ...
    file_name = 'Results/' + DATASET + '_Input_box_example_' + INPUT + '_' + checkpoint.split('/')[-1] + icl_name
        ...
        for project_name, data in dataset.items():
            ...
            for indx, row, row_embed in zip(range(len(data['test'])), data['test'], test_data_embed):
                if ICL_METHOD == 'task-level':
                    ...
                    top_n_items = get_the_most_relevant_items_for_an_item_given_cos_sim(indx, cos_sim, 900)
                    prompt = generate_prompt_by_top_n_items(INIT_PROMPT, row['prompt_context'], top_n_items,NUM_EXAMPLES_IN_PROMPT, data['train'])
                elif ICL_METHOD == 'instance-level-random':
