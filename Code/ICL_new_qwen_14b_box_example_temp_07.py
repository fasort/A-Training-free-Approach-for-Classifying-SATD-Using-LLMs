...
checkpoint = "/root/xinglin-data/falv/models/Qwen/Qwen2___5-14B-Instruct"

# model = LLM(model="/root/xinglin-data/satd/models/LLM-Research/gemma-2-27b-it",gpu_memory_utilization=0.9)
model = LLM(model="/root/xinglin-data/falv/models/Qwen/Qwen2___5-14B-Instruct",gpu_memory_utilization=0.99)
...
# generation_config = SamplingParams(temperature=0.01, max_tokens = 4096)
...
#     prompt += test_context + '\n'
#     prompt += 'There is only one type. You only need answer the type.<|im_end|>\n<|im_start|>assistant\n'
#     # prompt += 'There is only one type. First, provide the explanation after [exp], and then specify the type after [type].<|im_end|>\n<|im_start|>assistant\n'
#     return prompt
