...
# df = pd.read_csv('/root/xinglin-data/satd/newData/OBrien_with_exp_zhipu_only_comment.csv')
df = pd.read_csv('/root/xinglin-data/satd/Dataset/23_Shades/1_OBrien_with_exp_zhipu_fp+ct+cmb.csv')
print(len(df))
print(df.columns)
...
def generate_prompt_by_top_n_items(init_prompt, test_context, top_n_items, data):
    ...
    for indx,similarity in top_n_items:
        if len(split_to_tokens(prompt+data['prompt_context'][indx]+test_context))<8100:
...
for NUM_EXAMPLES_IN_PROMPT in num_instances:
    ...
    file_name = 'Results/' + DATASET + '_Input-exp-others-gemma-exp-' + INPUT + '_' + checkpoint.split('/')[-1] + icl_name
    ...
    with open(file_name+'_confmat.txt', "w") as output_file:
        for project_name, data in dataset.items():
            ...
            for indx, row, row_embed in zip(range(len(data['test'])), data['test'], test_data_embed):
                ...
                if len(split_to_tokens(prompt))<8100:
                    print(prompt)
