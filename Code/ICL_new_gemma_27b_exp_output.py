...
from transformers import DataCollatorWithPadding,AutoModelForSequenceClassification, Trainer, TrainingArguments,AutoTokenizer,AutoModel,AutoConfig,GemmaTokenizerFast
...
df = pd.read_csv('/root/xinglin-data/satd/newData/labeled_dataset_origin.csv')
print(len(df))
print(df.columns)
...
df = df[['context','prompt_context','Category','Project']]
...
checkpoint = "/root/xinglin-data/satd/models/LLM-Research/gemma-2-27b-it"

model = LLM(model="/root/xinglin-data/satd/models/LLM-Research/gemma-2-27b-it",gpu_memory_utilization=0.9)

tokenizer = GemmaTokenizerFast.from_pretrained("/root/xinglin-data/satd/models/LLM-Research/gemma-2-27b-it")
...
# REQUIREMENT: Requirement debt comments express incompleteness of the method, class or program.
...
def generate_prompt_by_top_n_items(init_prompt, test_context, top_n_items, data):
    ...
    for indx,similarity in top_n_items:
        if len(split_to_tokens(prompt+data['prompt_context'][indx]+test_context))<4058:
            prompt += data['prompt_context'][indx] + '\n'
            prompt += '### Label: ' + data['label'][indx] + '\n\n'
    # prompt += 'Give the type of following software technical debt:\n'
    prompt += 'Give the type of technical debt based on the following information:\n'
    prompt += test_context + '\n'
    # prompt += 'You only need answer the type.<end_of_turn>\n<start_of_turn>model\n'
    # prompt += 'There is only one type. First, provide the explanation after [exp], and then specify the type after [type].<end_of_turn>\n<start_of_turn>model\n'
    prompt += 'You should provide the explanation first, followed by the type. The explanation should be prefixed with \'[exp]:\' in a single line, and the type should be prefixed with \'[type]:\' in a separate line containing only one type label.<end_of_turn>\n<start_of_turn>model\n'

    # prompt += 'There is only one type. First, provide an explanation in one line after \'[exp]:\', and then specify the type in one line after \'[type]:\'.<end_of_turn>\n<start_of_turn>model\n'
    # prompt += 'There is only one type. You answer should following the required formate. First, provide an explanation in one line after \'[exp]:\', and then specify the type in one line after \'[type]:\'.<end_of_turn>\n<start_of_turn>model\n'
    # prompt += 'There is only one type. Your answer should follow the required format: First, provide a one-line explanation prefixed with \'[exp]:\', and then specify the type in a separate line prefixed with \'[type]:\'.<end_of_turn>\n<start_of_turn>model\n'
    return prompt
...
for NUM_EXAMPLES_IN_PROMPT in num_instances:
    ...
    file_name = 'Results/' + DATASET + '_exp_output2_Input_' + INPUT + '_' + checkpoint.split('/')[-1] + icl_name
        ...
        for project_name, data in dataset.items():
            ...
            for indx, row, row_embed in zip(range(len(data['test'])), data['test'], test_data_embed):
                ...
                if len(split_to_tokens(prompt))<4608:
                    ...
                    print(pred)
                    print(pred.lower().split('[type]')[-1])
                    for label in labels:
                        if label.lower() in pred.lower().split('[type]')[-1]:
                            pred = label
                            break
                else:
                    pred = ''
