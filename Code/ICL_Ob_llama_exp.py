...
df = pd.read_csv('/root/xinglin-data/satd/newData/OBrien_with_exp_zhipu.csv')
print(len(df))
print(df.columns)
...
def generate_prompt_by_top_n_items(init_prompt, test_context, top_n_items, data):
    ...
    for indx,similarity in top_n_items:
        if len(split_to_tokens(prompt+data['prompt_context'][indx]+test_context))<2048:
...
for NUM_EXAMPLES_IN_PROMPT in num_instances:
    ...
    file_name = 'Results/' + DATASET + '_Input-exp-' + INPUT + '_' + checkpoint.split('/')[-1] + icl_name
