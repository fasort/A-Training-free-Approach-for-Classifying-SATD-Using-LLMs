...
model = LLM(model="/root/xinglin-data/satd/models/LLM-Research/gemma-2-27b-it",gpu_memory_utilization=0.95)
...
def generate_prompt_by_top_n_items(init_prompt, test_context, top_n_items, data):
    ...
    for indx,similarity in top_n_items:
        if len(split_to_tokens(prompt+data['prompt_context'][indx]+test_context))<2560:
...
if 'instance' in ICL_METHOD:
    if len(INIT_PROMPT)>0:
        ...
        # num_instances = [20,21,22,23,24,25]
    else:
