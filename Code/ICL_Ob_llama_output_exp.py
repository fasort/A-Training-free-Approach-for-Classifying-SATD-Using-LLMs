...
# INPUT = 'ct+cm'                 # comment text + commit message
...
checkpoint = "/root/xinglin-data/Meta-Llama-3.1-8B-Instruct"

model = LLM(model="/root/xinglin-data/Meta-Llama-3.1-8B-Instruct",gpu_memory_utilization=0.9)



generation_config = SamplingParams(temperature=0.7, top_p = 0.95 , max_tokens = 512)
...
def generate_prompt_by_top_n_items(init_prompt, test_context, top_n_items, data):
    ...
    # prompt += test_context + '\n'
    # prompt += 'You only need answer the type.<|eot_id|>\n<|start_header_id|>assistant<|end_header_id|>\n'
    # prompt += 'Give the type of following technical debt:\n'
    # prompt += test_context + '\n'
    # prompt += 'You only need give the type.<|eot_id|>\n<|start_header_id|>assistant<|end_header_id|>\n'
    # prompt += 'Give the type of technical debt based on the following information:\n'
    # prompt += test_context + '\n'
    # prompt += 'There is only one type. You only need give the type.<|eot_id|>\n<|start_header_id|>assistant<|end_header_id|>\n'
    prompt += 'Give the type of technical debt based on the following information:\n'
    prompt += test_context + '\n'
    # prompt += 'There is only one type. First, provide the explanation after [exp], and then specify the type after [type].<|eot_id|>\n<|start_header_id|>assistant<|end_header_id|>\n'
    prompt += 'There is only one type. First, provide the explanation after \'[exp]:\' in one line, and then specify the type after \'[type]:\' in one line.<|eot_id|>\n<|start_header_id|>assistant<|end_header_id|>\n'
    return prompt
...
for NUM_EXAMPLES_IN_PROMPT in num_instances:
    ...
    file_name = 'Results/' + DATASET + '_new_prompt_Input_output_exp_' + INPUT + '_' + checkpoint.split('/')[-1] + icl_name
    ...
    with open(file_name+'_confmat.txt', "w") as output_file:
        for project_name, data in dataset.items():
            ...
            for indx, row, row_embed in zip(range(len(data['test'])), data['test'], test_data_embed):
                ...
                if len(split_to_tokens(prompt))<5000:
                    # print(prompt)
                    ...
                    # elif '*' in pred:
                    #     pred_group = re.search(r'\*+(.*?)\*+', pred)
                    #     pred = pred_group.group(1)
                    # elif '\n' in pred:
