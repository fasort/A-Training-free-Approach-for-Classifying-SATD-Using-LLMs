...
def generate_prompt_by_top_n_items(init_prompt, test_context, top_n_items, data):
    ...
    prompt += 'Give the type of technical debt based on the following information:\n'
    prompt += test_context + '\n'
    prompt += 'There is only one type. First, provide the explanation after \'[exp]:\' in one line, and then specify the type after \'[type]:\' in one line.<end_of_turn>\n<start_of_turn>model\n'
    return prompt
